...
def recommend_talks(talk_content, data=df):
    cluster = kmeans.predict(scaler.transform(
        pca.transform(vectorizer.transform(talk_content).toarray())))
    sim_in_cluster = get_similar_talks_in_cluster(
        talk_content, cluster[0], data)
...
# Visualize clusters in 2D space using PCA for dimensionality reduction
pca_2d = PCA(n_components=2)
X_pca_2d = pca_2d.fit_transform(X_scaled)

# Plot the clusters
plt.scatter(X_pca_2d[:, 0], X_pca_2d[:, 1], c=df['cluster'], cmap='viridis')
plt.title('TED Talk Clusters')
